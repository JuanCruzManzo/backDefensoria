...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($titulo === '' || strlen($titulo) > 30) {
        header("Location: index.php?vista=noticias/cargarMemoria&error=1"); exit;
    }
    if ($sub_titulo === '' || strlen($sub_titulo) > 50) {
        header("Location: index.php?vista=noticias/cargarMemoria&error=2"); exit;
    }
    if (!isValidDate($fecha_pub)) {
        header("Location: index.php?vista=noticias/cargarMemoria&error=3"); exit;
    }
    if ($estado !== 0 && $estado !== 1) {
        header("Location: index.php?vista=noticias/cargarMemoria&error=4"); exit;
    }
    if (!$usuario_id) {
        header("Location: index.php?vista=noticias/cargarMemoria&error=5"); exit;
    }
    ...
    if ($archivo && $archivo['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if (!$archivo_ruta) {
            header("Location: index.php?vista=noticias/cargarMemoria&error=6"); exit;
        }
    }
    ...
    if ($memoria_id > 0) {
        if ($archivo_ruta) {
            ...
            mysqli_stmt_bind_param($stmt, 'sssissi',
                $titulo, $sub_titulo, $archivo_ruta, $estado, $fecha_pub, $usuario_id, $memoria_id
            );
        } else {
            $stmt = mysqli_prepare(
                $link,
            ...
            mysqli_stmt_bind_param($stmt, 'ssissi',
                $titulo, $sub_titulo, $estado, $fecha_pub, $usuario_id, $memoria_id
            );
        }
        ...
        header("Location: index.php?vista=noticias/cargarMemoria&error=11"); exit;

    } else {
        if (!$archivo_ruta) {
            header("Location: index.php?vista=noticias/cargarMemoria&error=12"); exit;
        }
        ...
        mysqli_stmt_bind_param($stmt, 'sssissi',
            $titulo, $sub_titulo, $archivo_ruta, $estado, $fecha_crea, $fecha_pub, $usuario_id
        );
        $ok = mysqli_stmt_execute($stmt);
        ...
        header("Location: index.php?vista=noticias/cargarMemoria&error=13"); exit;
    }
} else {
